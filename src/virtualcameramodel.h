...
class VirtualCameraModel : public QAbstractListModel {
    Q_OBJECT
public:
    enum CameraRoles { NameRole = Qt::UserRole + 1, SourceUrlRole, CameraUrlRole, IsActiveRole };

    explicit VirtualCameraModel(VideoFrameProvider *provider, QObject *parent = nullptr);

    [[nodiscard]] int rowCount(const QModelIndex &) const override;
    [[nodiscard]] QHash<int, QByteArray> roleNames() const override;
    [[nodiscard]] QVariant data(const QModelIndex &index, int role) const override;
    Q_INVOKABLE [[nodiscard]] QVariantMap get(int row) const;
    Q_INVOKABLE int addCamera(const QString &name,
